...
if(isset($image)){
    ...
    }else{move_uploaded_file($image_tmp_name, $image_folder);
    }
    }else{
        $image='';
    }
...
if (isset($_POST['draft'])){
    ...
    $select_image=$conn->prepare("select *  FROM `poducts` WHERE image =? AND seller_id=?");
    $select_image->execute([$image,$seller_id]);
    if(isset($image)){
        if($select_image->rowCount()>0){
            $warning_msg[]='image name repeated';
...
<head>
    ...
    <title>Blue Sky Summer -admin and products page</title>
...
<div class="input-field">
    ...
    <input type="number" name="price" maxlength="100" placeholder="add product name" required class="box">
    </div>
    <div class="input-field">
